...
public class CustomerFilterService {
    ...
    public CustomerFilterResponse addCustomerFilter(AddCustomerFilterRequest request) {
        if (request == null) throw new IllegalArgumentException("Request required");
        return mapper.response(repository.insertAfter(request.id(), request.label(), request.parentKey(), request.after()));
    }
}
