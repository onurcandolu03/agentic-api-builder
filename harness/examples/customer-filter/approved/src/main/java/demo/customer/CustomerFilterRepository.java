...
public class CustomerFilterRepository {
    ...
    public synchronized CustomerFilter insertAfter(String id, String label, String parentKey, String after) {
        if (id == null || id.isBlank() || label == null || label.isBlank()
                || parentKey == null || parentKey.isBlank() || after == null || after.isBlank())
            throw new IllegalArgumentException("All request fields are required");
        if (rows.stream().anyMatch(r -> r.id().equals(id))) throw new IllegalArgumentException("Duplicate filter key");
        int order = positionAfter(parentKey, after);
        List<CustomerFilter> next = new ArrayList<>();
        for (CustomerFilter row : rows) next.add(row.parentKey().equals(parentKey) && row.filterOrder() >= order
                ? row.withOrder(row.filterOrder() + 1) : row);
        CustomerFilter inserted = new CustomerFilter(id, label, parentKey, order);
        next.add(inserted);
        rows = next;
        return inserted;
    }
